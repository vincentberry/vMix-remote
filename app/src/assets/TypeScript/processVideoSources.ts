/**
 * Processes video sources from an XML document and updates the DOM accordingly.
 *
 * The function retrieves video source inputs from the provided XML document and checks for an existing
 * container with the ID "videoSourcesContainer". It iterates over each video source and either updates an
 * existing DOM element (identified by a unique key) or creates a new element for the source. If the source
 * type is "Mix" and the title is formatted correctly, additional HTML generated by getVideoMixSourceHTML is appended.
 * After processing all sources, any DOM element representing a video source that is no longer present in the
 * XML document is removed.
 *
 * @param xmlDoc - The XML document containing video source inputs.
 */
...
/**
 * Generates HTML markup for a video source using its XML element attributes.
 *
 * The returned string includes a clickable main area that sends a preview command based on the video source's key, as well as overlay buttons for additional commands. CSS classes are conditionally applied to the video source and its overlays based on the numeric value of the "number" attribute relative to defined state variables.
 *
 * @param videoSource - An XML element representing the video source. Expected to have attributes such as "key", "title", "number", and "type".
 * @returns The HTML string for rendering the video source and its overlay actions.
 */
...
/**
 * Generates HTML markup for a video mix source from the provided XML data.
 *
 * This function extracts input source details and mix configuration from the given XML document using the specified mix number. It builds dropdowns for program and preview selections, marking the currently active and previewed inputs, and constructs a mix transition control section with a "CUT" button and a transition options dropdown. The dropdown options exclude the input source identified by the provided input key.
 *
 * @param xmlDoc - The XML document containing input and mix source elements.
 * @param MixNumber - The identifier used to locate the mix source element in the XML.
 * @param Inputkey - The key associated with the current input source to be excluded from the options.
 * @returns The HTML string representing the video mix source controls.
 *
 * @remark This function depends on a globally defined variable, Alltransition, for populating the transition options.
 */
